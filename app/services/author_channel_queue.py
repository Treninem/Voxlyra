...
async def ensure_author_channel_queue_schema() -> None:
    async with connect() as db:
        ...
        await db.execute("CREATE INDEX IF NOT EXISTS idx_author_channel_queue_due ON author_channel_queue(status, next_attempt_at)")
        await db.commit()
...
async def update_author_channel_settings(*, enabled: int | None = None, interval_minutes: int | None = None, posts_per_run: int | None = None) -> dict[str, int]:
    ...
    if interval_minutes is not None:
        ...
        next_run = (datetime.now(timezone.utc) + timedelta(minutes=current["interval_minutes"])).replace(microsecond=0).isoformat()
        async with connect() as db:
            await db.execute("UPDATE author_channel_queue SET next_attempt_at=? WHERE status='queued'", (next_run,))
            await db.commit()
...
async def retry_failed_author_posts() -> int:
    await ensure_author_channel_queue_schema()
    async with connect() as db:
        cur = await db.execute("UPDATE author_channel_queue SET status='queued', attempts=0, next_attempt_at=?, last_error=NULL WHERE status='failed'", (utc_now(),))
        await db.commit()
        return int(cur.rowcount or 0)


async def process_author_channel_queue(bot) -> int:
    from app.services.cross_platform_publication import post_book_everywhere
    ...
    for row in rows:
        ...
        try:
            result = await post_book_everywhere(bot, book_id, actor_user_id=actor_id, force=False)
            if result.channel_status in {"sent", "already_sent", "not_configured"}:
                async with connect() as db:
