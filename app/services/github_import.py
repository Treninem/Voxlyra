...
def validate_manifest(data: dict[str, Any], *, package_path: str, commit_sha: str) -> GitHubPackage:
    ...
    return GitHubPackage(package_id, content_type, str(data["title"]).strip(), str(data["language"]).strip(),
                         str(data["version"]).strip(), str(data["created_at"]).strip(), files, checksums,
                         package_path, commit_sha)


async def ensure_github_import_schema() -> None:
    async with connect() as db:
        await db.execute("""CREATE TABLE IF NOT EXISTS github_import_history(
            id INTEGER PRIMARY KEY AUTOINCREMENT, package_id TEXT NOT NULL, content_type TEXT NOT NULL,
            title TEXT NOT NULL DEFAULT '', version TEXT NOT NULL, commit_sha TEXT NOT NULL,
            status TEXT NOT NULL, file_count INTEGER NOT NULL DEFAULT 0, bytes_total INTEGER NOT NULL DEFAULT 0,
            book_id INTEGER, error TEXT, created_at TEXT NOT NULL,
...
async def discover_packages(identity_id: int, *, page: int = 1, page_size: int | None = None) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=20.0, follow_redirects=False) as client:
        for content_type, folder in _TYPE_DIRS.items():
            path = _root_path(folder)
            entries = await _get_json(client, f"https://api.github.com/repos/{quote(owner)}/{quote(repo)}/contents/{quote(path, safe='/')}", {"ref": branch})
            if not entries:
                continue
            for entry in entries:
                ...
                manifest = await _get_json(client, f"https://api.github.com/repos/{quote(owner)}/{quote(repo)}/contents/{quote(package_path + '/manifest.json', safe='/')}", {"ref": commit_sha})
                if not manifest or manifest.get("type") != "file":
                    continue
                raw = await client.get(manifest["download_url"], headers=_headers())
                raw.raise_for_status()
                package = validate_manifest(raw.json(), package_path=package_path, commit_sha=commit_sha)
                if package.content_type != content_type:
                ...
                if previous:
                    ...
                    package.status = "imported" if package.current_version == package.version else "update"
                found.append(package)
    found.sort(key=lambda item: (item.content_type, item.package_id))
    start = max(0, (max(1, int(page)) - 1) * size)
    selected = found[start:start + size]
...
async def download_package(identity_id: int, package: GitHubPackage) -> Path:
    require_system_owner(identity_id)
    owner, repo = _repo()
    temp_root = Path(str(settings.GITHUB_IMPORT_TEMP_ROOT or "storage/github_import"))
    ...
    try:
        async with httpx.AsyncClient(timeout=None, follow_redirects=False) as client:
            for name in package.files:
                api = f"https://api.github.com/repos/{quote(owner)}/{quote(repo)}/contents/{quote(package.path + '/' + name, safe='/')}"
                meta = await _get_json(client, api, {"ref": package.commit_sha})
...
async def record_import(package: GitHubPackage, *, status: str, book_id: int | None = None,
                        bytes_total: int = 0, error: str = "") -> None:
    await ensure_github_import_schema()
    ...
    async with connect() as db:
        await db.execute("""INSERT OR REPLACE INTO github_import_history(
            package_id, content_type, title, version, commit_sha, status, file_count, bytes_total, book_id, error, created_at
        ) VALUES(?,?,?,?,?,?,?,?,?,?,?)""", (package.package_id, package.content_type, package.title, package.version,
            package.commit_sha, status, len(package.files), int(bytes_total), book_id, safe_error, utc_now()))
        await db.commit()
