...
class VoskSegmentProvider:
    ...
    Модель загружается лениво и один раз на процесс. В Docker-образе она
    предварительно скачивается во время сборки, поэтому владельцу не требуется
    отдельный сервер, URL, API-ключ или ручная установка.
    """
        ...
        with cls._load_lock:
            ...
            try:
                ...
                if model_path.exists():
                    ...
                    # Запасной автоматический путь. В штатном Docker-образе модель
                    # уже скачана при сборке, но локальный запуск тоже остаётся автономным.
                    model = Model(model_name=str(settings.TTS_VOSK_MODEL_NAME))
                synth = Synth(model)
                cls._model = model
                cls._synth = synth
                cls._last_error = ''
