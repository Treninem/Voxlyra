    ...
    if command == "author":
        ...
        if profile:
            ...
            if not identities.get("linked"):
                text += " При привязке укажите @username или ID второго аккаунта и подтвердите запрос там — VoxLyra объединит данные автоматически."
        return (text, _vk_app_and_commands_keyboard("author", "✍ Открыть кабинет автора", [[("🔗 Связать Telegram + VK", "link")], [("🏠 Главное меню", "main")]]))
    if command == "link":
        return (
            "🔗 Один аккаунт VoxLyra\n\n"
            "Откройте настройки и введите @username или ID своего Telegram-аккаунта. "
            "VoxLyra отправит подтверждение в Telegram-бот. После подтверждения покупки, баланс, библиотека и прогресс объединятся автоматически. "
            "По одному username или ID данные не сливаются.",
            _vk_app_and_commands_keyboard("settings", "🔗 Открыть привязку аккаунтов", [[("⬅ Моё", "my")], [("🏠 Главное меню", "main")]]),
        )
    if command == "support":
