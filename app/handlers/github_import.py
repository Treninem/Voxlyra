...
from app.services.github_import import GitHubImportError,discover_packages,import_history,repository_status,import_package
router=Router()
...
async def protected_placeholder(call):
 if not _allowed(call):return await _deny(call)
 await call.answer("Эта операция будет включена после полного теста пакетной обработки",show_alert=True)
